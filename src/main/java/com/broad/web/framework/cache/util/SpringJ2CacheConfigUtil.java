package com.broad.web.framework.cache.util;
...
public class SpringJ2CacheConfigUtil {
    ...
    public final static J2CacheConfig initFromConfig(StandardEnvironment environment) {
        ...
        String l2ConfigSection = environment.getProperty("j2cache.L2.config_section");
        if (l2ConfigSection == null || l2ConfigSection.trim().equals("")) {
            l2ConfigSection = config.getL2CacheName();
        }
        String l2Section = l2ConfigSection;
        ...
        environment.getPropertySources().forEach(a -> {
            if (a instanceof MapPropertySource) {
                ...
                c.getSource().forEach((k, v) -> {
                    ...
                    if (key.startsWith(l2Section + ".")) {
                        config.getL2CacheProperties().setProperty(key.substring((l2Section + ".").length()),
                                environment.getProperty(key));
                    }
                });
            }
        ...
        if (config.getL1CacheProperties().isEmpty() || config.getL2CacheProperties().isEmpty() || config.getBroadcastProperties().isEmpty()) {
            environment.getPropertySources().forEach(ps -> {
                ...
                setProperty(config, environment, l2Section, propertyNames);
            });
        }
        return config;
    }

    private static void setProperty(J2CacheConfig config, StandardEnvironment environment, String l2Section, String[] propertyNames) {
